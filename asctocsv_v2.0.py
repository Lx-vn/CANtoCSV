...
# 解码数据并保存到 CSV
def decode_and_save_to_csv(parsed_data, dbc, output_csv_path, now_file, length):
...
# 主程序
def convert_asc_to_csv(asc_file_path, dbc_file_path, output_csv_path, now_file, length):
    # 加载 DBC 文件
    ...
    # 解码并保存为 CSV
    data_dict = decode_and_save_to_csv(parsed_data, dbc, output_csv_path, now_file, length)
...
# 遍历dbc文件夹
dbc_file = list_dbc_files(dbc_directory)
...
for i in asc_files:
    ...
    convert_asc_to_csv(asc_file_path, dbc_file_path, output_csv_path, now_file, length)
    now_file += 1
print('转换完成！')